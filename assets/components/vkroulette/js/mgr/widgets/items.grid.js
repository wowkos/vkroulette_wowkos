vkroulette.grid.Items = function(config) {
	config = config || {};
	Ext.applyIf(config,{
		id: 'vkroulette-grid-items'
	...
	vkroulette.grid.Items.superclass.constructor.call(this,config);
};
Ext.extend(vkroulette.grid.Items,MODx.grid.Grid,{
	windows: {}

	,getMenu: function() {
		var m = [];
		m.push({
			text: _('vkroulette_item_update')
			,handler: this.updateItem
		});
		m.push('-');
		m.push({
			text: _('vkroulette_item_remove')
	...
	,createItem: function(btn,e) {
		if (!this.windows.createItem) {
			this.windows.createItem = MODx.load({
				xtype: 'vkroulette-window-item-create'
				,listeners: {
				...
				success: {fn:function(r) {
					if (!this.windows.updateItem) {
						this.windows.updateItem = MODx.load({
							xtype: 'vkroulette-window-item-update'
							,record: r
							,listeners: {
								'success': {fn:function() { this.refresh(); },scope:this}
							}
	...
	,removeItem: function(btn,e) {
		...
		MODx.msg.confirm({
			title: _('vkroulette_item_remove')
			,text: _('vkroulette_item_remove_confirm')
			,url: this.config.url
			,params: {
...
Ext.reg('vkroulette-grid-items',vkroulette.grid.Items);
...
vkroulette.window.CreateItem = function(config) {
	...
	Ext.applyIf(config,{
		title: _('vkroulette_item_create')
		,id: this.ident
		,height: 200
		,width: 475
		,url: vkroulette.config.connector_url
...
Ext.reg('vkroulette-window-item-create',vkroulette.window.CreateItem);
...
vkroulette.window.UpdateItem = function(config) {
	...
	Ext.applyIf(config,{
		title: _('vkroulette_item_update')
		,id: this.ident
		,height: 200
		,width: 475
		,url: vkroulette.config.connector_url
...
Ext.reg('vkroulette-window-item-update',vkroulette.window.UpdateItem);